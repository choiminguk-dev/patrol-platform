...
- 작업사진(쓰레기/도로/사람만 있는 사진)만 있으면 그 그룹의 주소를 비워두세요.
...
- 주소판이 없어도 배경이 달라지면 분할하세요. 주소를 비워두면 됩니다.
- 하나의 그룹은 보통 **2~5장**입니다. 6장 이상이면 분할 포인트를 놓친 건 아닌지 재확인하세요.

★★★ 사진 watermark 시간 — 그룹 분할의 **최우선** 단서 (다른 모든 규칙보다 우선):
- 사진 하단·구석에 watermark 로 timestamp 가 박혀있는 경우가 많습니다 (예: "07:05", "09:27", "10:12", "2026.05.16 09:05").
- watermark 시간을 **반드시 읽고**, 그룹 분할의 최우선 단서로 사용하세요.
- 같은 그룹의 사진들은 watermark 시간이 **5분 이내**여야 합니다.
- 5분 이상 갭 → **무조건 분할**. 배경이 유사해도, 같은 주소판 앞이어도, 같은 도로명이어도 **반드시 분할**.
- 이 규칙은 "주소판 앞의 사진은 같은 그룹" 규칙보다 **우선**합니다.
- 예시:
  사진 21(07:06), 22(07:05), 23(09:05 주소판 '후암로4길 28')
  → 21·22 는 23 과 시간갭 약 2시간 → **다른 그룹**.
  → 23 단독으로 그룹 형성 (주소판만 단독 금지 규칙도 watermark 시간 우선시 예외).
  사진 29(09:27), 30(10:12)
  → 갭 45분 → **다른 그룹**.
- watermark 가 없거나 읽을 수 없는 사진은 이 규칙 적용 안 함 (다른 분할 단서 사용).
- reasoning 필드에 watermark 시간을 적으세요 (예: "사진 21=07:06, 22=07:05, 23=09:05 → 갭 2시간 → 21·22 와 23 분할").
...
JSON 형식으로만 응답:
{"groups":[{"address":"도로명 건물번호","photoIndices":[1,2,3],"photoTimes":{"1":"07:05","2":"07:06","3":null},"description":"(07:05) 좁은 골목 청소 작업","zoneSeqNo":1,"reasoning":"사진 3에서 파란 표지판 '두텁바위로 155' 확인. 사진 1=07:05, 2=07:06 → 갭 1분 → 동일 그룹."}]}

- photoIndices는 사진 라벨의 번호를 그대로 사용하세요.
- **photoTimes (필수)**: 각 사진의 watermark timestamp 를 "HH:MM" 형식으로 명시. 키는 photoIndices 의 인덱스(문자열), 값은 시간 또는 null.
  - watermark 가 명확히 보이면 "HH:MM" (예: "07:05", "10:12").
  - watermark 가 없거나 안 보이면 null (추측 금지).
  - **모든 사진이 watermark 가 없어도 photoTimes 객체 자체는 반드시 포함하세요 — 모든 키를 null 로 채운 dict 라도 OK.** 필드 누락 금지.
  - 사진 좌하단/우상단/모서리에 작은 디지털 시계 글씨 (예: "2024-05-17 14:32" / "07:05" / "오후 2:30") 가 있는지 반드시 확인. 한국어/영문/숫자 모두 검토.
- **description**: 현장 상황 한줄 설명. watermark 시간이 하나라도 보이면 "(HH:MM) 한줄설명" 형식으로 시간 prefix 의무
  (예: "(06:44) 좁은 주택가 골목 청소 작업"). 시간 prefix 가 있어야 dashboard 목록 뷰에서 사용자가 그룹 분할 이유를 즉시 식별 가능.
  watermark 가 전혀 없는 그룹만 prefix 생략.
- zoneSeqNo는 상습지역 구역 목록이 제공된 경우에만 포함 (연번 숫자 또는 null).
- 구역 목록에 없거나 확실하지 않으면 zoneSeqNo: null.
- reasoning: 1~2문장으로 어떤 사진의 무엇을 보고 이 주소/그룹을 결정했는지 기록 (현장 안전관리자가 읽을 한국어 자연문장으로). watermark 시간이 있으면 reasoning 에 명시. 코드/필드명("address", "빈 문자열" 등) 사용 금지 — "주소판 없음", "주소를 비워둠" 같은 사용자 언어 사용.`;
    ...
    for (let chunkStart = 0; chunkStart < totalPhotos; chunkStart += CHUNK_SIZE) {
      ...
      try {
        ...
        if (parsed.groups?.length > 0) {
          // photoTimes 첫 non-null 시각 → group.firstWatermarkHHMM 보존.
          // EXIF strip 된 batch (카톡 송수신 등) 의 entry.originalPhotoTime 폴백으로 사용.
          for (const g of parsed.groups) {
            const pt = g.photoTimes;
            if (!pt || typeof pt !== "object") continue;
            for (const idx of (g.photoIndices || [])) {
              const v = pt[idx] ?? pt[String(idx)];
              if (typeof v === "string" && /^\d{1,2}:\d{2}$/.test(v.trim())) {
                g.firstWatermarkHHMM = v.trim();
                break;
              }
            }
          }
        ...
        console.log(`[analyze] chunk ${chunkStart + 1}-${chunkEnd}: AI returned ${parsed.groups?.length || 0} groups:`,
          JSON.stringify((parsed.groups || []).map((g: any) => ({ addr: g.address, photos: g.photoIndices, wm: g.firstWatermarkHHMM }))));
